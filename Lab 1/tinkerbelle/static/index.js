...
// Create the "blues" button
const bluesBtn = document.createElement('button');
bluesBtn.id = 'blues';
bluesBtn.innerText = 'Blues';

document.body.appendChild(bluesBtn);

bluesBtn.onclick = () => {
  socket.emit('blues');
};

socket.on('blues', (imagePath) => {
...
// Create the "jazz" button
const jazzBtn = document.createElement('button');
jazzBtn.id = 'jazz';
jazzBtn.innerText = 'Jazz';

document.body.appendChild(jazzBtn);

jazzBtn.onclick = () => {
  socket.emit('jazz');
};

socket.on('jazz', (imagePath) => {
  document.body.style.backgroundImage = `url(${imagePath})`;
  document.body.style.backgroundColor = 'transparent'; // Reset the background color
});
